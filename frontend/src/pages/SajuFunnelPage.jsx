...
// 🔴 정직한 범위(2026-08-12 갱신 — 아래 3줄은 낡아서 실제 코드와 안 맞았음, 코코가 대조 후 정정):
//   입력→백엔드 /api/saju 실연동됨. 4대운 피크 전용 엔진(saju_fortune_curve.py)은 대박운·조심시기·
//   연애운·결혼운 4카드 전부 헤드라인연도+그래프가 실데이터 연동됨(spoSpecs 함수 주석 참고, 77d9200).
//   reading 문단(구체 서사)만 엔진이 안 주는 정보라 4카드 공통 데모 카피(정직게이트 유지).
//   결제는 OrderModal → RTPay 무통장 자동매칭 웹훅(orders.py)으로 연동돼 있고, unified 상품 PDF
//   미발송 버그도 수정됨(a689b00) — "버튼만 있고 미연동"은 더 이상 사실이 아니다.
