...
PRODUCTS = {
    ...
    # 🔴 2026-08-12 형 확정: "고삼타로 서비스 가격 9,900원" — 사주/타로 나누지 않고 통일.
    "tarot_spread": {"name": "타로 스프레드 전체 풀이", "amount": 9900,
                      "desc": "뽑은 스프레드 전체 카드 뜻 + 질문 종합 풀이 PDF"},
    # 🔴 코코 추가(2026-08-12): 사주+타로 통합 흐름(SajuFunnelPage 유형선택→카드뽑기→UnifiedPaywall) 상품.
    #   08-11 형 확정 — 사주 따로/타로 따로가 아니라 하나의 흐름, 결제도 끝에 한 번(₩9,900).
    "unified": {"name": "사주 x 카드 합친 결론", "amount": 9900,
                "desc": "사주가 보는 큰 흐름 + 카드가 짚는 지금이 수렴하는 통합 결론 — 정확한 시기·행동·처방 PDF"},
...
ADMIN_TOKEN = os.environ.get("ADMIN_TOKEN", "")   # 자동매칭 실패 시 수동승인용
...
@router.post("/api/orders/{order_id}/admin_approve")
def admin_approve(order_id: str, request: Request):
    """자동매칭(웹훅·bankapi) 실패 시 운영자 수동 승인. 실계좌 확인 후에만 호출할 것."""
    # 🔒 fail-closed: ADMIN_TOKEN 미설정이면 엔드포인트 자체 차단. 상수시간 비교로 타이밍공격 차단.
    if not ADMIN_TOKEN:
        raise HTTPException(503, "admin approve not configured")
    ...
    with SessionLocal() as db:
        ...
        if order.status == "paid":
            return {"status": "paid"}
        _approve(db, order, "manual_admin_approve")
        return {"status": "paid"}
