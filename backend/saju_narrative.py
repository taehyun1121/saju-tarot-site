...
def extended_report_sections(engine_pillars, daeun: list, gender: str, birth_year: int) -> dict:
    ...
    # 디자인봇 히어로카드 필드계약(good/warn: name/count/note, favor_years=최장 연속년수)
    LOVE_NOTE = {"정관": "안정적 인연·책임있는 관계", "정재": "성실하고 오래가는 인연",
                 "편관": "강렬하나 굴곡있는 인연", "편재": "자유롭고 변화많은 인연"}
    yeonae = {
        "good": [{"name": good_key, "count": counts.get(good_key, 0), "note": LOVE_NOTE[good_key]}],
        "warn": [{"name": care_key, "count": counts.get(care_key, 0), "note": LOVE_NOTE[care_key]}],
        "favor_years": max([r["years"] for r in good_runs], default=0),
    }
    ...
    return {
        ...
        # 보조(디자인봇 히어로카드가 요약만 보여줘서, 상세 시기 목록은 별도 유지 — 기존 report_saju.html 섹션에서 계속 사용)
        "seun_table": seun_table, "gyeok_detail": gyeok_detail,
        "good_love_runs": good_runs, "care_love_runs": care_runs,
