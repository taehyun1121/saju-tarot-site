...
from saju_rule_engine import full_report, daewoon_direction, sipseong_counts
...
def extended_report_sections(engine_pillars, daeun: list, gender: str, birth_year: int) -> dict:
    ...
    # ③ 세운 연도별 전체(3~83세, 81개) — 서술이 아니라 표 형태(문단 81개는 과함)
    curve = score_curve(engine_pillars, gender, birth_year,
                         daewoon_su=(daeun[0]["start"] if daeun else None), age_range=(3, 83))["curve"]
    seun_table = [{"age": c["age"], "year": c["year"], "ganji": c["sewoon"], "score": round(c["score"])} for c in curve]
    ...
    return {"full_daeun": full_daeun, "sipseong_full": sipseong_full,
