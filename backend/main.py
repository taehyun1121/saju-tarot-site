...
① 위치별 흐름 서술 (각 포지션이 말하는 것)
- 각 카드 포지션을 순서대로 언급하며 "이 자리에 [카드명]이 나왔다는 것은..." 형식으로 서술
- 각 위치가 질문 맥락에서 무엇을 의미하는지 구체적으로 설명
- 정방향/역방향이 해당 포지션에서 어떤 에너지로 작동하는지 포함

② 위치 간 연결 (카드들이 함께 말하는 것)
- 서로 연결되는 포지션 쌍 또는 흐름을 명시적으로 연결
  예: "현재 상황의 [A]와 장애물인 [B]를 함께 보면...", "[C] 자리와 [D] 자리가 공명하여..."
- 카드들 사이에 긴장·조화·강화 관계가 있으면 반드시 짚어주세요
- 사주 흐름(대운·세운)이 이 카드 구도와 어떻게 맞물리는지 연결

③ 종합 결론
- 질문에 직접 답하는 한 문장으로 시작
- 시간적 맥락이 있으면 반영
- 이 사람의 사주 기질에 맞는 구체적 조언으로 마무리

분량: 10-15문장. 상담사가 그대로 읽어줄 수 있는 수준으로 작성"""
...
def generate_ai_tarot_reading(result_cards: list, spread_name: str, question: str = "", saju_context: dict = None) -> dict:
    ...
    pos_list = " / ".join([f"{c['position_name']}({c['card_name']})" for c in result_cards])
    overall_guide = (
        f'① 각 포지션별 흐름: [{pos_list}] 순서로 위치가 말하는 것 서술 '
        f'② 위치 간 연결: 카드들 사이 긴장·조화·강화 관계 명시 '
        f'③ 종합 결론: "{question}"에 직접 답하고 사주 기질에 맞는 조언으로 마무리 '
        f'— 10-15문장'
        if question else
        f'① 각 포지션별 흐름: [{pos_list}] 순서로 서술 '
        f'② 위치 간 연결: 카드들 사이 관계 명시 '
        f'③ 종합 결론: 사주 기질에 맞는 조언 — 10-15문장'
    )
